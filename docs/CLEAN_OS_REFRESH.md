...
**Local panel vs first Wi‑Fi:** stock MainsailOS still boots to a text login until postinstall runs. First network join still needs a one-time path (USB keyboard + `nmtui`, Ethernet, or [boot-partition Wi‑Fi](https://docs.mainsail.xyz/mainsailos/faq/change-wifi-credentials/)). After `postinstall-magneto.sh`, **HelixScreen** owns the HDMI panel and its **first-run wizard** includes Wi‑Fi + Moonraker setup ([HelixScreen](https://github.com/prestonbrown/helixscreen)).
